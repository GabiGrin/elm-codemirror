...
Elm.CodeMirror.make = function (_elm) {
   "use strict";
   _elm.CodeMirror = _elm.CodeMirror || {};
   if (_elm.CodeMirror.values) return _elm.CodeMirror.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $Native$CodeMirror = Elm.Native.CodeMirror.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm);
   var _op = {};
   var codeMirror$ = $Native$CodeMirror.codeMirror;
   var codeMirror = F3(function (config,msgCreator,code) {    return A2(codeMirror$,{cmConfig: config,value: code},msgCreator);});
   var CmConfig = F5(function (a,b,c,d,e) {    return {theme: a,mode: b,height: c,lineNumbers: d,lineWrapping: e};});
   var Config = F2(function (a,b) {    return {value: a,cmConfig: b};});
   return _elm.CodeMirror.values = {_op: _op,codeMirror: codeMirror,CmConfig: CmConfig};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $CodeMirror = Elm.CodeMirror.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var _op = {};
   var update = F2(function (acc,model) {
      var _p0 = acc;
      switch (_p0.ctor)
   ...
   var view = F2(function (add,model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "display",_1: "flex"}]))]),
      _U.list([container(_U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Instance 1")]))
                                 ,A3($CodeMirror.codeMirror,cmConfig(model),function (_p1) {    return A2($Signal.message,add,CodeChange(_p1));},model.code)]))
              ,container(_U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Instance 2")]))
                                 ,A3($CodeMirror.codeMirror,cmConfig(model),function (_p2) {    return A2($Signal.message,add,CodeChange(_p2));},model.code)]))
              ,container(_U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text("Controls")]))
                                 ,A2($Html.div,
                                 _U.list([]),
                                 _U.list([$Html.text("Theme:")
                                         ,A2($Html.input,
                                         _U.list([$Html$Attributes.placeholder("Theme")
                                 ...
                                 ,A2($Html.div,
                                 _U.list([]),
                                 _U.list([$Html.text("Mode (elm or javascript are loaded)")
                                         ,A2($Html.input,
                                         _U.list([$Html$Attributes.placeholder("Mode")
                                                 ,$Html$Attributes.value(model.mode)
                                                 ,A3($Html$Events.on,
                                                 "input",
                                                 $Html$Events.targetValue,
                                                 function (_p4) {
                                 ...
                                 _U.list([$Html.text("\"raw\" code")
                                         ,A2($Html.textarea,
                                         _U.list([$Html$Attributes.placeholder("Code")
